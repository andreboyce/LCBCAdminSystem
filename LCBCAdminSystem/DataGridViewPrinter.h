...
// This code was utilized because of its robustness and functionality would have been
// Hard to duplicate in the short space of time we had to complete the project

#ifndef __DataGridViewPrinter_H
#define __DataGridViewPrinter_H
...
public ref class DataGridViewPrinter
{
	private: System::Windows::Forms::DataGridView^ TheDataGridView; // The DataGridView Control which will be printed
	...
	private: System::String^ TheTitleText; // The title text to be printed in each page (if IsWithTitle is set to true)
	...
	private: System::Collections::Generic::List<array<int>^ >^ mColumnPoints;
    private: System::Collections::Generic::List<float>^ mColumnPointsWidth;
    private: int mColumnPoint;
        
    // The class constructor
	public: DataGridViewPrinter( DataGridView^ aDataGridView, PrintDocument^ aPrintDocument, bool CenterOnPage, bool WithTitle, System::String^ aTitleText, Font^ aTitleFont, Color aTitleColor, bool WithPaging)
    {
        ...
        RowsHeight = gcnew List<float>();
        ColumnsWidth = gcnew List<float>();

        mColumnPoints = gcnew List<array<int>^>();
        mColumnPointsWidth = gcnew List<float>();
    ...
    private: void Calculate( Graphics^ g )
    {
        if( PageNumber == 0 ) // Just calculate once
        {
            ...
            for (int i = 0; i < TheDataGridView->Columns->Count; i++)
            {
                ...
                if( tmpFont == nullptr ) // If there is no special HeaderFont style, then use the default DataGridView font style
                ...
                for( int j = 0; j < TheDataGridView->Rows->Count; j++ )
                {
                    ...
                    if( tmpFont == nullptr ) // If the there is no special font style of the CurrentRow, then use the default one associated with the DataGridView control
                        tmpFont = TheDataGridView->DefaultCellStyle->Font;
            ...
            if (TheDataGridViewWidth > mTempPrintArea)
            {
                ...
                for( k = 0; k < TheDataGridView->Columns->Count; k++ )
                {
                    if( TheDataGridView->Columns[k]->Visible )
                    {
                        ...
                        // If the width is bigger than the page area, then define a gcnew column print range
                        if( mTempWidth > mTempPrintArea )
                        {
                            mTempWidth -= ColumnsWidth[k];
                            mColumnPoints->Add( gcnew array<int> { mStartPoint, mEndPoint } );
    ...
    private: void DrawHeader( Graphics^ g )
    {
        ...
        {
            PageNumber++;
			System::String^ PageString = gcnew System::String( "" );
			PageString = "Page " + PageNumber.ToString();

            StringFormat^ PageStringFormat = gcnew StringFormat();
			PageStringFormat->Trimming = StringTrimming::Word;
			PageStringFormat->FormatFlags = StringFormatFlags::NoWrap | StringFormatFlags::LineLimit | StringFormatFlags::NoClip;
			PageStringFormat->Alignment = StringAlignment::Far;

			Font^ PageStringFont = gcnew Font( "Tahoma", 8, FontStyle::Regular, GraphicsUnit::Point );

            RectangleF PageStringRectangle = RectangleF( (float)LeftMargin, CurrentY, (float)PageWidth - (float)RightMargin - (float)LeftMargin, g->MeasureString( PageString, PageStringFont ).Height );

			g->DrawString( PageString, PageStringFont, gcnew SolidBrush(Color::Black), PageStringRectangle, PageStringFormat );
        ...
        if( IsWithTitle )
        {
            ...
            RectangleF TitleRectangle = RectangleF( (float)LeftMargin, CurrentY, (float)PageWidth - (float)RightMargin - (float)LeftMargin, g->MeasureString( TheTitleText, TheTitleFont ).Height );

            g->DrawString( TheTitleText, TheTitleFont, gcnew SolidBrush(TheTitleColor), TitleRectangle, TitleFormat );
        ...
        SolidBrush^ HeaderForeBrush = gcnew SolidBrush( HeaderForeColor );
        ...
        SolidBrush^ HeaderBackBrush = gcnew SolidBrush(HeaderBackColor);

        // Setting the LinePen that will be used to draw lines and rectangles (derived from the GridColor property of the DataGridView control)
        Pen^ TheLinePen = gcnew Pen(TheDataGridView->GridColor, 1);

        // Setting the HeaderFont style
        Font^ HeaderFont = TheDataGridView->ColumnHeadersDefaultCellStyle->Font;
        if( HeaderFont == nullptr ) // If there is no special HeaderFont style, then use the default DataGridView font style
            HeaderFont = TheDataGridView->DefaultCellStyle->Font;

        // Calculating and drawing the HeaderBounds        
        RectangleF HeaderBounds = RectangleF( CurrentX, CurrentY, mColumnPointsWidth[mColumnPoint], RowHeaderHeight );
        g->FillRectangle( HeaderBackBrush, HeaderBounds );

        // Setting the format that will be used to print each cell of the header row
        StringFormat^ CellFormat = gcnew StringFormat();
        ...
        for( int i = (int)mColumnPoints[mColumnPoint]->GetValue(0); i < (int)mColumnPoints[mColumnPoint]->GetValue(1); i++ )
        {
            ...
            CellBounds = RectangleF( CurrentX, CurrentY, ColumnWidth, RowHeaderHeight );
    ...
    private: bool DrawRows( Graphics^ g )
    {
        ...
        while (CurrentRow < TheDataGridView->Rows->Count)
        {
            if( TheDataGridView->Rows[CurrentRow]->Visible ) // Print the cells of the CurrentRow only if that row is visible
            {
                ...
                if (RowFont == nullptr) // If the there is no special font style of the CurrentRow, then use the default one associated with the DataGridView control
                ...
                RowForeBrush = gcnew SolidBrush(RowForeColor);
                ...
                {
                    RowBackBrush = gcnew SolidBrush( TheDataGridView->DefaultCellStyle->BackColor );
                    RowAlternatingBackBrush = gcnew SolidBrush(TheDataGridView->AlternatingRowsDefaultCellStyle->BackColor);
                }
                ...
                {
                    RowBackBrush = gcnew SolidBrush(RowBackColor);
                    RowAlternatingBackBrush = gcnew SolidBrush(RowBackColor);
                }
                ...
                RowBounds = RectangleF(CurrentX, CurrentY, mColumnPointsWidth[mColumnPoint], RowsHeight[CurrentRow]);
                ...
                for (int CurrentCell = (int)mColumnPoints[mColumnPoint]->GetValue(0); CurrentCell < (int)mColumnPoints[mColumnPoint]->GetValue(1); CurrentCell++)
                {
                    ...
                    RectangleF CellBounds = RectangleF(CurrentX, CurrentY, ColumnWidth, RowsHeight[CurrentRow]);
